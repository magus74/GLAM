#ifndef VIC_OOC_TRIANGLE_MESH_HPP
#define VIC_OOC_TRIANGLE_MESH_HPP
...
namespace vic {
  ...
  public:
    ooc_triangle_mesh(uint_t nverts = 0,uint_t ntris = 0,std::string tmpfname = "",size_type_t cache_size = 10*1024*1024);
    ~ooc_triangle_mesh();
  ...
  protected:
    sl::external_array1<point_t>        *mesh_vertex_position_;
    sl::external_array1<color_t>        *mesh_vertex_color_;
    sl::external_array1<dual_vector_t>  *mesh_vertex_normal_;
    sl::external_array1<value_t>        *mesh_vertex_quality_;
    sl::external_array1<tri_t>          *mesh_triangles_;

  };

  inline ooc_triangle_mesh::uint_t ooc_triangle_mesh::vertex_count() const {
    return mesh_vertex_position_->size();
  }

  inline ooc_triangle_mesh::uint_t ooc_triangle_mesh::triangle_count() const {
    return mesh_triangles_->size();
  }

  inline void ooc_triangle_mesh::resize_triangle(uint_t ntris) {
    mesh_triangles_->resize(ntris);
  }

  inline void ooc_triangle_mesh::resize_vertex(uint_t nverts) {
  ...
  inline void ooc_triangle_mesh::push_vertex(const point_t& p,
					     const color_t& c,
  ...
  inline void ooc_triangle_mesh::push_triangle(const tri_t& t) {
