...
namespace vic {
  ...
  typedef vic::ooc_triangle_mesh::uint_t                            uint_t;
  ...
  void ooc_ply_io::to_file(ooc_triangle_mesh* tm, const char* str) {
    //-------------------------------//
    PlyFile* out_ply = 0;
    int nelems = 2;
  ...
  ooc_triangle_mesh* ooc_ply_io::from_file(const char* str, float scale) {
    //    std::cout << "____TEST____" << std::endl;
    ooc_triangle_mesh* result = 0;
    PlyFile* in_ply = 0;
    FILE* f = fopen(str, "r");  
    if (f == NULL) {
    ...
    if (in_ply) {
      ...
      result = new ooc_triangle_mesh(0,0,str);
	...
	if (equal_strings ("vertex", elem_name)) {
	  ...
	  for (j = 0; j < nverts; j++) {
	    ...
	    if (v.x<MIN_COORD_VALUE || v.x>MAX_COORD_VALUE || v.y<MIN_COORD_VALUE || v.y>MAX_COORD_VALUE || v.z<MIN_COORD_VALUE || v.z>MAX_COORD_VALUE) {
	      ...
	      result->set_position(j,point_t(v.x * scale, v.y * scale ,v.z * scale));
	      result->set_color(j,color_t(v.red/255.0f,v.green/255.0f,v.blue/255.0f));
	      result->set_normal(j,dual_vector_t(v.nx,v.ny,v.nz));
	      result->set_quality(j,float(v.quality));
	    }
	  }
	} else if (equal_strings ("face", elem_name)) {
	  /* create a list to hold all the face elements */
	  nfaces = elem_count;
	  result->resize_triangle(nfaces);
	  ...
	  for (j = 0; j < nfaces && !corrupted_file; j++) {
		...
		result->set_triangle(j,tri_t(f.verts[0],f.verts[1],f.verts[2]));
		...
		if (f.verts[0] < 0 || f.verts[0] >= nverts ||  
		  ...
		  const tri_t t = result->get_triangle(j);
		  const point_t p0 = result->get_position(t[0]);
		  const point_t p1 = result->get_position(t[1]);
		  const point_t p2 = result->get_position(t[2]);
		  ...
		  if (dv_norm>1e-5) {
		    ...
		    for(std::size_t idx = 0; idx<3; ++idx) {
		      dual_vector_t n0 = result->get_normal(t[idx]);
		      n0 += dv;
		      result->set_normal(t[idx],n0);
		    }
		  }
#else
		  ...
		  if (w0) {
		    dual_vector_t n0 = result->get_normal(t[0]);
		    n0 += dual_vector_t(float(dv[0]/w0),float(dv[1]/w0),float(dv[2]/w0));
		    result->set_normal(t[0],n0);
		  }
		  if (w1) {
		    dual_vector_t n1 = result->get_normal(t[1]);
		    n1 += dual_vector_t(float(dv[0]/w1),float(dv[1]/w1),float(dv[2]/w1));
		    result->set_normal(t[1],n1);
		  }
		  if (w2) {
		    dual_vector_t n2 = result->get_normal(t[2]);
		    n2 += dual_vector_t(float(dv[0]/w2),float(dv[1]/w2),float(dv[2]/w2));
		    result->set_normal(t[2],n2);
		  }
#endif
		} 
	      } 
		...
		result->resize_triangle(result->triangle_count()+1);
		nfaces = result->triangle_count();
		result->set_triangle(j,tri_t(f.verts[0],f.verts[1],f.verts[3]));
		j+=1;
		result->set_triangle(j,tri_t(f.verts[1],f.verts[2],f.verts[3]));
	      }
#endif
	      free(f.verts);
	    }
	  ...
	  if (!corrupted_file) {
	    ...
	    for(uint_t i = 0; i < result->vertex_count(); ++i) {
	      dual_vector_t n = result->get_normal(i);
  ...
  void ooc_ply_io::apply_transform(const char* str_dest,
				   const char* str_src,
				   rigid_body_map_t& R,
				   rigid_body_map_t& T) {
    ooc_triangle_mesh* tm = vic::ooc_ply_io::from_file(str_src);
    ...
    vic::ooc_ply_io::to_file(tm,str_dest);
