...
void get_binary_item(int, FILE *, int, int *, unsigned int *, double *);
...
static void convert16(char* c) {
  char tmp[2]; int i;
  for (i=0; i<2; ++i) tmp[i] = c[1-i];
  for (i=0; i<2; ++i) c[i] = tmp[i];
}

static void convert32(char* c) {
  char tmp[4]; int i;
  for (i=0; i<4; ++i) tmp[i] = c[3-i];
  for (i=0; i<4; ++i) c[i] = tmp[i];
}

static void convert64(char* c) {
  char tmp[8]; int i;
  for (i=0; i<8; ++i) tmp[i] = c[7-i];
  for (i=0; i<8; ++i) c[i] = tmp[i];
}
...
void binary_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {          /* list */
      ...
      /* get and store the number of items in the list */
      get_binary_item (((plyfile->file_type == PLY_BINARY_LE) != is_little_endian()),
                       fp, prop->count_external,
                      &int_val, &uint_val, &double_val);
      if (store_it) {
        item = elem_data + prop->count_offset;
        store_item(item, prop->count_internal, int_val, uint_val, double_val);
...
void write_scalar_type (FILE *fp, int code)
{
  ...
  //RH substituted type_names with old_type_names
  ...
  switch (type) {
    ...
    case Uint8:
      ...
      *double_val = *uint_val;
      break;
    case Int16:
      fread (ptr, 2, 1, fp); if (convert_endian) convert16((char*)ptr); 
      *int_val = *((short int *) ptr);
      *uint_val = *int_val;
      *double_val = *int_val;
      break;
    case Uint16:
      ...
      *double_val = *uint_val;
      break;
    case Int32:
      fread (ptr, 4, 1, fp); if (convert_endian) convert32((char*)ptr);
      *int_val = *((int *) ptr);
      *uint_val = *int_val;
      *double_val = *int_val;
      break;
    ...
    case Float32:
      ...
      *uint_val = *double_val;
      break;
    case Float64:
      fread (ptr, 8, 1, fp); if (convert_endian) convert64((char*)ptr);
      *double_val = *((double *) ptr);
      *int_val = *double_val;
      *uint_val = *double_val;
      break;
...
#else
  random_pick = (int) floor ( rules->nprops * ( (float)(rand()%1000000) / (float)(1000000-1) ) );
#endif
